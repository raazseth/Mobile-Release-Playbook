...
- New prompt `ai/prompts/aso-optimization.md`: App Store Optimization prompt for store listing keywords, title/subtitle/description copy, and Product Page Optimization / Store Listing Experiments A/B test variants — the business/growth side of store metadata, distinct from `ai/prompts/metadata.md`'s policy-compliance-only scope. Cross-linked from `metadata.md`, `ai/prompts/README.md`, `templates/app-store-description.md`, and `templates/play-store-description.md`.
- Added an "AI-Assisted Draft Generation" section to every file in `templates/` (`release-plan.md`, `release-checklist.md`, `rejection-response.md`, `release-notes.md`, `changelog.md`, `privacy-questionnaire.md`, `incident-report.md`, `app-store-description.md`, `play-store-description.md`): a prompt that turns raw release facts into a first draft of that specific template. Where a matching prompt already existed in `ai/prompts/` (release notes/changelog generation, rejection response drafting, ASO/metadata copy), the template points to it instead of duplicating it; `release-plan.md`, `release-checklist.md`, `privacy-questionnaire.md`, and `incident-report.md` get new template-specific prompts, since nothing in `ai/prompts/` already covered those. The privacy-questionnaire prompt is explicit that every row is `UNKNOWN` until a human verifies it — privacy declarations are legally binding, not documentation prose.
...
- Rewrote `.github/pull_request_template.md`: removed the internal-only repository-instructions reference and the AI-tool-specific `search_web` jargon (meaningless to a human contributor), and replaced the checklist with one that points to `CONTRIBUTING.md` / `governance/documentation-style.md`, prompts contributors to state which directory a change belongs in and what overlap they checked for, and adds a collapsible AI-safety checklist for PRs touching `ai/`.
