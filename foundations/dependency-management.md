...
`npm ci` requires the lockfile to match `package.json` and does not update the lockfile during installation. This makes it useful for reproducible CI installs. citeturn0search16
...
Expo documents `npx expo install` as the supported way to install Expo SDK packages and notes that it installs versions compatible with the project's Expo SDK. citeturn0search4

Expo CLI also supports npm, Yarn, pnpm, and Bun package managers. citeturn0search2
...
Expo Doctor checks areas including app configuration, `package.json`, dependency compatibility, and project health. citeturn0search3
...
Expo documents this as a CI-friendly check that exits non-zero when incompatible package versions are detected. citeturn0search2
...
React Native's current upgrade guidance also recommends incremental Expo SDK upgrades because they make breakages easier to identify. citeturn0search0
...
React Native's Upgrade Helper shows the changes between two React Native versions and helps identify required native project changes. citeturn0search0
...
Expo documents that `npx expo install` can automatically add some config plugins to static app configuration, but dynamic configuration may require manual changes. citeturn0search5
...
For Expo projects, Expo Doctor can check packages against the React Native Directory by default. citeturn0search3
...
For React Native specifically, the project uses a `0.x.y` versioning policy where breaking changes are shipped in new minor versions. Do not assume standard semantic-versioning intuition applies directly to React Native. citeturn0search18
...
The project's security guidance treats third-party packages, lockfiles, dependency updates, CI/CD permissions, build secrets, and untrusted scripts as supply-chain security concerns. fileciteturn9file10L1-L20
...
Expo documents that libraries requiring custom native code may require a development build rather than Expo Go. citeturn0search12
...
For React Native upgrades, use the current React Native Upgrade Helper and apply the required native changes. citeturn0search0
...
Expo documents `npx expo install --check` as suitable for CI because it exits non-zero when package versions are incompatible. citeturn0search2
...
Expo documents these tools for identifying and correcting dependency-version mismatches. citeturn0search15turn0search2
